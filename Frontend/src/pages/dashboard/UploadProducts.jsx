...
const CarouselSlick = lazy(() => import("../../components/CarouselSlick"));
...
import UploadProductsCategories from "../../components/UploadProductsCategories";
import UploadProductsSubCategories from "../../components/UploadProductsSubCategories";
...
const UploadProducts = () => {
	...
	const [coverImage, setCoverImage] = useState({ name: "", url: "", image: null });
	...
	async function handleImportData(e) {
		...
		if (file && file.type === "application/json") {
			...
			function updateDataStates(data) {
				...
				if (moreDetails && Object.keys(moreDetails)[0]) {
	...
	const isDisabled = () => {
		let isDisabled = Object.entries(form).some((entry) => {
			let v = entry[1]; // form field value
			let n = entry[0]; // form field label

			if (typeof v === "object" && n === "subCategories") {
				return false;
	...
	const handleSubmit = async (e) => {
		...
		const images = [coverImage?.image,...form.images];
		...
		// 2nd Way (difficulty in resetting categories and subcategories)
		// setForm((prev) => {
		// 	return {
		// 		...prev,
		// 		name: "",
		...
		// reset.selectCategories()
		// reset.selectSubCategories()
		// setUploadedImages((prev) => {
		// 	return {
		// 		...prev,
		// 		images: [],
		...
		// });
		// setAddMoreDetails({});
		// // Clear memory of images previews after form reset
		// if (uploadedImages.previews[0]) {
		// 	uploadedImages.previews.forEach((url) => {
	...
	}, [allCategories, allSubCategories, reset]);
	useEffect(() => {
	...
	return (
		<section className="_uploadProducts min-h-[80vh] w-full pb-4 relative z-0">
			{/* Top Part */}
			...
			<div className="_bottomPart px-3 ">
				...
				>
					...
					{addMoreDetails && addMoreDetails[0] &&
						addMoreDetails.map((d, i, ar) => {
							const detailsName = d?.name;
							const id = d?.id;
								...
								<>
									<div className="_more_details  space-y-1.5 grid m-4">
										...
										<button
											...
											onClick={(e) => handleAddMoreDetails(e)}
										>
											Add
										</button>
									</div>
					...
					<div className="_submitBtn">
						<button
							...
							disabled={isDisabled()}
						>
							{!uploading ? "Add New Product" : <AnimatingDots text={"Adding"} />}
						</button>
					</div>
					{disabled && (
